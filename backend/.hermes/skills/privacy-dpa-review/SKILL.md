...
1. 加载 `./CLAUDE.md` → DPA 操作手册。如为占位符，停止并提示设置。
...
DPA 有两种形态，审查方向几乎完全相反。当客户发来他们的 DPA，我们在捍卫我们的运营灵活性。当我们给供应商发 DPA，我们在保护我们（及我们客户的）数据。两次审查都读同一份 `./CLAUDE.md` 操作手册，但从相反的行读取。

## 首先：哪个方向？

做任何事之前，先确定：

- **我们是受托处理者** → 客户向我们发送他们的 DPA → 读取 `./CLAUDE.md` → "当我们是受托处理者时" 表（对应个保法第21条委托处理 `[法条原文]`）
...
审查前，检查输出文件夹中关于本对方当事人或处理活动的先前工作。读取 `./CLAUDE.md` → `## 输出` 获取输出文件夹路径。扫描：
...
**从上游继承严重程度作为底线**，遵循 `./CLAUDE.md` → `## 共享护栏` 中的跨技能严重程度底线规则。被分诊评为🔴的处理活动不能在 DPA 审查中静默降级为🟢；任何降级均应说明和解释。

如果未找到先前的输出（新对方当事人/新活动），在审查中明确说明——"输出文件夹中无关于本对方当事人的先前分诊或PIA"——以便审核律师知道检查已执行。

## 加载操作手册

读取 `./CLAUDE.md` → `## DPA 操作手册`。同时读取 `## 个人信息处理规则承诺`——DPA 不能与处理规则承诺相矛盾。
...
逐条审查 DPA 的以下条款。*具体*的数值和实体立场（通知期限、泄露时限、可接受/不可接受底线）来自 `./CLAUDE.md` → `## DPA 操作手册`。任何 DPA 必须满足的监管底线来自主源法律——**检索每个适用制度的现行有效规则**并引用主源后再陈述底线。
...
冠以 `./CLAUDE.md` `## 输出` 中的工作成果抬头（因用户角色不同而异——见 `## 谁在使用`）。
...
**在签署 DPA（包括返回已签署版本、在对方平台上同意自动执行、或指示签字人签署）之前：** 读取 `./CLAUDE.md` 中的 `## 谁在使用`。如果角色为非律师：
