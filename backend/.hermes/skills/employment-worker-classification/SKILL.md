---
name: employment-worker-classification
description: >
...
读取 `./CLAUDE.md` → 管辖范围、任何既有的认定历史或先前争议记录、升级表，以及团队记录的任何内部用工政策。
...
如果 `./CLAUDE.md` 记录了公司内部认定政策，先适用该政策，并标识与研究标准之间的任何冲突。
...
**在做出"可以按劳务关系/劳动关系/派遣/外包进行"的最终建议前：** 读取 `./CLAUDE.md` 中的 `## Who's using this`。如果角色是**非律师**：
