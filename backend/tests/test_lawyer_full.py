...
测试身份设定：陈立恒律师（恒远律师事务所 合伙人，执业 12 年，民商事诉讼 + 常年顾问）。
...
MATTERS = [
    {
        "id": "m-hx", "title": "恒信建材诉宏达建设买卖合同纠纷", "client": "恒信建材有限公司",
        "counterparty": "宏达建设工程有限公司", "opposingCounsel": "郑国辉",
    ...
    {
        "id": "m-lg", "title": "王建国诉天工机械劳动争议", "client": "天工机械制造有限公司",
        "counterparty": "王建国", "opposingCounsel": "刘文杰",
    ...
    {
        "id": "m-lh", "title": "张淑芬诉李国栋离婚纠纷", "client": "张淑芬（女方）",
        "counterparty": "李国栋（男方）", "opposingCounsel": "",
    ...
    {
        "id": "m-zx", "title": "恒信建材申请执行宏达建设", "client": "恒信建材有限公司",
        "counterparty": "宏达建设工程有限公司", "opposingCounsel": "",
    ...
    {
        "id": "m-bg", "title": "恒信建材收购明远物流股权", "client": "恒信建材有限公司",
        "counterparty": "明远物流有限公司", "opposingCounsel": "",
    ...
    {
        "id": "m-ct", "title": "（演示）某公司股东知情权纠纷", "client": "某公司",
...
PROFILE = {
    "name": "陈立恒", "firm": "恒远律师事务所", "title": "合伙人",
...
async def section_c(ws):
    ...
    try:
        ...
        ws_.append(["(2026)京0105民初8273号", "恒信建材", 3850000])
        wb.save(buf)
    ...
    docs = [
        ...
        {"name": "送货单汇总.txt", "text": "2026年3月至6月共送货 7 批，累计钢材 486 吨，均由宏达建设材料员签收。"},
        {"name": "往来函件.txt", "text": "恒信建材于2026年7月15日致函宏达建设催告付款，宏达建设至今未支付剩余货款 385 万元。"},
    ]
    r, _ = await rpc(ws, "kb_search", {"matter_id": "m-hx", "query": "宏达建设还欠多少货款没有支付", "documents": docs, "top_k": 3}, timeout=60)
    hits = r.get("chunks", [])
    ...
    r, _ = await rpc(ws, "export_docx", {"title": "民事起诉状", "markdown": md, "matter_title": "恒信建材诉宏达建设"}, timeout=60)
...
async def section_d(ws):
    ...
    archived_alerts = [a for a in alerts if "李明" in str(a.get("matter", ""))]
...
async def section_e(ws):
    ...
    r, _ = await rpc(ws, "chat", {
        "message": "我方代理原告恒信建材，被告宏达建设拖欠货款 385 万元已逾期。请简要说明诉讼时效起算点，并列出我方应准备的三项核心证据。控制在 200 字内。",
    ...
    r, _ = await rpc(ws, "chat", {
        ...
        "matter_documents": [
            ...
            {"name": "往来函件.txt", "text": "恒信建材于2026年7月15日致函催告，宏达建设至今未支付剩余货款 3,850,000 元。"},
        ],
    }, timeout=300)
    resp = r.get("response", "")
    ...
    r, _ = await rpc(ws, "chat", {
        ...
        "experience_items": [
            {"id": "e1", "matterTitle": "恒信建材诉宏达建设买卖合同纠纷", "type": "decision",
             "title": "是否申请财产保全", "text": "决策：立即申请诉前财产保全，冻结宏达建设基本户。理由：对方有转移资产迹象。"},
        ],
    }, timeout=300)
    resp = r.get("response", "")
    hit = "保全" in resp
    ...
    try:
        ...
        if os.path.exists(user_md):
            with open(user_md, "r", encoding="utf-8") as fh:
                md = fh.read()
            record("F6", "setup_save 写入律师画像 USER.md", "陈立恒" in md and "恒远律师事务所" in md,
                   f"命中姓名+律所={'是' if ('陈立恒' in md and '恒远律师事务所' in md) else '否'}")
        else:
