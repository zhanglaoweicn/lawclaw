...
**扫描模式**（无参数或 `--sweep`）：
1. 读取 `./CLAUDE.md` → 输出文件夹路径、处理规则文件、上次扫描日期。
...
6. 更新 `./CLAUDE.md` 中的上次处理规则扫描日期。

**直接查询模式**（带描述参数）：
1. 读取 `./CLAUDE.md` → 当前处理规则承诺 + 实际处理规则文件。
...
完成扫描后将 **上次处理规则扫描** 更新为今天日期至 `./CLAUDE.md`。
...
每次扫描运行时，它更新 `./CLAUDE.md` 中 `## 输出` → **上次处理规则扫描**，这样下次扫描只看新文件。
