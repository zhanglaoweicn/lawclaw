...
| B. 便携目录 | 无 | U 盘/临时机器/不写注册表 | `portable/` 整目录（或 Release 上的 `*_portable_x64.zip`） |
| C. 从源码运行 | Python ≥3.11、Node ≥18、pnpm 9+ | 开发调试 | — |
| D. 从源码打包 | 方式 C + Rust + MSYS2 MinGW + NSIS | 出交付物 | 安装器 + 便携目录 |

> 安装包**已内嵌 Python 运行时**，目标机不需要装 Python（这个性质有构建期自检保证，
> 见本文末「运行时如何保证不依赖目标机」）。
>
> 本产品**只有一个版本**：首启空台账，**不含任何内置/测试案件数据，也不含任何密钥**。

---

## A. 安装器

1. 到 [Releases](https://github.com/zhanglaoweicn/lawclaw/releases) 下载 `LawClaw_x.y.z_x64-setup.exe`；
2. 双击安装，按向导完成；
3. 首次启动进入配置向导：
   - **第 1 步 API 配置**：填 API 地址 / 模型名 / API Key（任意 OpenAI 兼容端点），
     点「测试连接」验证通过后再继续——**必须先填 Key 才能进入主界面**；
   - 第 2 步律师信息（可跳过，但填了回答会更贴合你的业务）；
   - 第 3 步消息平台（可跳过）；
4. 进来是**空台账**，从「案件 → 新建案件」开始建档。
...
从 Release 下载 `LawClaw_x.y.z_portable_x64.zip` 解压，或使用本地构建产出的 `release/portable/`。
**整个目录拷到目标机任意位置，双击 `LawClaw.exe`**：
...
- **Python ≥ 3.11**（本项目在 **3.14** 上验证；`pyproject.toml` 里上游留下的
  ...
  见下方说明）
- **Node ≥ 18**（CI 用 22）+ **pnpm 9+**（`corepack enable pnpm` 或 `npm i -g pnpm`）
...
python -X utf8 backend/tests/test_lawyer_full.py    # 全场景 100+ 例（A~F 分区，可传 A B C… 只跑部分）
...
```bash
# 全量：payload + 捆绑运行时 + 前端 + NSIS 安装器 + 自检/冒烟
...
1. **不含任何案件数据**：仓库里没有演示/内置案件，首启即空台账；
2. **不含任何密钥**：构建脚本里**没有注入密钥的开关**，产物天然无密钥。
