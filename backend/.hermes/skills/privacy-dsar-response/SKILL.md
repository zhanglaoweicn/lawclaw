...
1. 加载 `./CLAUDE.md` → DSAR 流程（系统清单、验证方式、SLA）。
2. 执行以下工作流。
...
**在将任一函件发送给个人信息主体之前：** 读取 `./CLAUDE.md` 中的 `## 谁在使用`。如果角色为非律师：
...
> **注意：** 两份 DSAR 函件均为对外发送给个人信息主体的文件。**勿**在两份函件上包含 `./CLAUDE.md` `## 输出` 中的工作成果抬头。函件附随的内部笔记、日志和豁免分析是律师工作成果——将这些分开保存并冠以工作成果抬头。
...
如果 `./CLAUDE.md` → `## DSAR 流程` 记录了一个比法定期限更严格的内部 SLA，使用内部 SLA 并注明法定底线。
