...
class DaemonThreadPoolExecutor(ThreadPoolExecutor):
    ...
    def _adjust_thread_count(self) -> None:
        ...
        # - 3.14+: worker context objects (`_create_worker_context`, 3-arg _worker)
        # - 3.8–3.13: private `_initializer`/`_initargs` + 4-arg _worker
        #   (attributes removed in 3.14 — must NOT be touched there)
        ...
        if num_threads < self._max_workers:
            ...
            if create_ctx is not None:
                # CPython 3.14+: official spawn signature (worker context object)
                worker_args = (weakref.ref(self, weakref_cb), create_ctx(), self._work_queue)
            else:
                # CPython 3.8–3.13: carry initializer into the worker thread
