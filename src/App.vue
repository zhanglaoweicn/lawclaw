  ...
  <Transition name="root-fade" mode="out-in">
    ...
    <div class="app-footer">
        ...
        <div class="footer-links">
          <!-- 开发工具只在 dev 构建显示；演示版额外保留「重置演示数据」（演示前一键回到初始态） -->
          <template v-if="isDevBuild || isDemoBuild">
            <span class="seed-link" @click="seedData">{{ isDevBuild ? '恢复演示数据' : '重置演示数据' }}</span>
          </template>
...
async function seedData() {
  try {
    await ElMessageBox.confirm(
      '此操作将清除所有会话、案件和数据，恢复为演示数据。建议先导出备份。是否继续？',
      '确认恢复演示数据',
      { confirmButtonText: '确认清除并恢复', cancelButtonText: '取消', type: 'warning' }
    )
  } catch {
    return  // 用户取消
  }
