...
async def main():
    async with websockets.connect(URI, max_size=30 * 1024 * 1024) as ws:
        ...
        pdf_ok = r.get("ok") and "500万元" in r.get("text", "") and "测试法院" in r.get("text", "")
        ...
        r = await rpc(ws, "experience_search", {
            "query": "借贷合同纠纷 违约 诉讼",
            "items": [
                {"id": "1", "matterTitle": "测试委托人巳公司", "title": "测试委托人巳与测试相对方午借贷合同纠纷", "text": "民间借贷 500万元 违约 利息 诉讼", "type": "matter"},
                {"id": "2", "matterTitle": "测试当事人甲", "title": "测试案件·离婚纠纷", "text": "离婚 抚养权 财产分割", "type": "matter"},
                {"id": "3", "matterTitle": "测试委托人庚", "title": "发明专利侵权纠纷", "text": "专利侵权 二审 赔偿", "type": "matter"},
            ],
        })
        hits = r.get("hits", [])
